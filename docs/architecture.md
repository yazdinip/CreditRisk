...
1. **Data Layer & Ingestion (`ingest_data`)**
   - `python -m creditrisk.pipelines.ingest_data --config configs/creditrisk_pd.yaml` now speaks directly to Kaggle, S3, Azure Blob, or DVC remotes (based on each `ingestion.sources` entry), downloads/decompresses the raw tables, enforces MD5 checksums, and writes a bronze snapshot summary to `reports/ingestion_summary.json`.
...
2. **Feature Engineering (`build_feature_store`)**
   - CLI: `python -m creditrisk.pipelines.build_feature_store --config configs/creditrisk_pd.yaml`.
...
3. **Dataset Splitting (`split_data`)**
   - CLI: `python -m creditrisk.pipelines.split_data --config configs/creditrisk_pd.yaml`.
...
4. **Model Fitting (`fit_model`)**
   - CLI: `python -m creditrisk.pipelines.train_creditrisk_pd --config configs/creditrisk_pd.yaml --skip-artifacts`.
   - Responsibilities: load cached splits, rebalance (SMOTE + downsampling), fit the sklearn pipeline, serialize the model, and log to MLflow/registry while deferring evaluation artifacts to downstream stages.
5. **Evaluation (`evaluate_train` / `evaluate_test`)**
   - CLI: `python -m creditrisk.testing.test_dataset --split train|test`.
   - Responsibilities: score cached splits, persist metrics/plots/predictions, and provide regression evidence before validation/governance.
6. **Experiment Tracking**
   - MLflow logs parameters, metrics, artifacts, and registered versions. Local runs default to the `mlruns/` folder but any tracking URI can be configured in `configs/creditrisk_pd.yaml`.
6. **Serving / Deployment**
   ...
   - Promotion flow: the training stage emits `reports/registry_promotion.json`, CD invokes `python -m creditrisk.pipelines.auto_promote`, `python -m creditrisk.pipelines.canary_validation` compares the candidate model against the current Production artifact before ECS is touched, and `python -m creditrisk.pipelines.deploy_manifest` compiles all deployment hooks into `reports/deploy_manifest.json`.
...
split_data  (train/test parquet)
      ...
      |
      v
fit_model -> evaluate_train -> evaluate_test -> validate_model -> register_model -> post-training reports + MLflow + models
```
...
- **Config toggles** (`validation.*` in `configs/creditrisk_pd.yaml`) allow you to disable specific checks in exploratory environments without editing the pipeline code.
...
- **MLflow-backed registry**: when `registry.enabled` is true, every `fit_model` run registers its serialized pipeline under `registry.model_name`. Versions are automatically staged when they meet the configured metric threshold.
...
- **Local dev**: run the modular CLIs (`ingest_data`, `build_feature_store`, `split_data`, `fit_model`, `evaluate_train`, `evaluate_test`) directly or via `dvc repro`. Artifacts are tracked in DVC and MLflow, so you can iterate safely.
...
- **MLflow-backed registry**: when `registry.enabled` is true, every `fit_model` run registers its serialized pipeline under `CreditRiskPD`. Versions auto-stage when they meet the configured metric threshold.
